...
def setup_loaders(rotation_degrees: list[int], batch_size: int, n_eval_tasks: int = 3):
    test_tasks = rotation_degrees[:n_eval_tasks]
    train_loaders = dict()
    test_loaders = dict()
...
def train_model(model_name: str,
    ...
    train_loaders, test_loaders = setup_loaders(train_config['rotation_degrees'], train_config['batch_size'], train_config['n_eval_tasks'])
...
if __name__=='__main__':
    ...
    results = run_continual_learning({'model_name': 'BranchModel', 'n_b_1': 14, 'n_b_2': 14, 'rotation_degrees': [0, 270, 45, 135, 225, 350, 180, 315, 60, 150, 240, 330, 90], 
                                      'epochs_per_task': 4, 'batch_size': 32, 'soma_func': 'relu', 'device': device, 'n_repeat': 0, 
                                      'sparsity': 0.0, 'learn_gates': False, 'debug': True, 'lr': 0.01,
                                      'file_path': './branchNetwork/data/SomeFunc/', 'file_name': 'test_relu', 'l2': 0.0})
    time_end = time.time()
