...
def run_tune():
    ...
    layer_1_branches =  [1,2,7,14, 28, 49, 98, 196, 382, 784] # [14] # [49,98,196,392,784] # [1,2,7,14,] 
    ...
    if 'talapas' in socket.gethostname():
        path = '/home/mtrappet/branchNetwork/data/Rotate_LongSequence_talapas/soma_func_softmax_sum/'
    else:
        path = '/home/users/MTrappett/mtrl/BranchGatingProject/data/Rotate_LongSequence/soma_func_softmax_sum/'
    param_config = BASE_CONFIG
    param_config['file_path'] = path
    param_config['model_name'] = tune.grid_search(MODEL_NAMES)
    param_config['n_repeat'] = tune.grid_search([i for i in range(repeats)]) # tune.grid_search([0, 2, 3, 4])
    ...
    param_config['soma_func'] = tune.grid_search(['max']) # , 'softmax_sum' # lets do softmax_sum after with different temps # 'sum', 'max', 'softmax', 'softmax_sum'
    # param_config['temp'] = tune.grid_search([0.1, 1.0, 10.0])
